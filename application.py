...
class MyApplication(object):
    ...
    def init_info(self):
        ...
        ttk.Label(self.root, text="使用说明\n" +
                                  ...
                                  "然后选中任意一个人，初始默认的都是我的初始数据，需要重新填写的所有offer工资(合同款项）、工资占比(0-1)、\n" +
                                  "出勤天数、补贴、报销、公积金比例（讲道理不用改）、基础薪金（用来计税的）、\n" +
                                  "实发报销工资（如果填的数额太大了，重新计算的时候会自动调小，一旦这个填了，保险部分的转账也会自己算出来）\n" +
                                  "然后第一时间点确认并重新计算，确认没错可以导出当前人的excel，或者是切换到其他人，全部编辑完再一起导出\n" +
                                  "选择一起导出的时候，还会附带一个总表，把所有编辑过的人（已经计算过的list里面，没点重新计算的不会放里面），都放在一个表里面\n"+
                                  "如果有任何bug，都是我的锅").grid(row=14, column=0, columnspan=5, rowspan=10)
